...
function Header() {
  ...
  return (
    <header className="backdrop-blur-sm bg-white/70 border-b border-slate-200 sticky top-0 z-20">
      <div className="mx-auto flex max-w-5xl items-center justify-between px-4 py-3">
        <div className="flex items-center gap-2 font-semibold text-slate-900">
          ...
          <div>
            ...
            <div className="text-xs text-slate-500">Memorize by looping yourself</div>
          </div>
        </div>
        <nav className="flex items-center gap-3 text-sm font-medium text-slate-600">
          <span className={location.pathname === '/' ? 'text-indigo-600' : ''}>Library</span>
          <span className={location.pathname.startsWith('/record') ? 'text-indigo-600' : ''}>Record</span>
        </nav>
      </div>
    </header>
  )
